...
M = l**2 * np.dot(Mb, Mf)       # dot multiplication to obtain M
...
plt.plot(np.linspace(0, modes-1, modes), lam, '-')              # plots numerical solution
plt.plot(np.linspace(0, modes-1, modes), 4/modearray, '*')      # plots analytical solution
plt.show()